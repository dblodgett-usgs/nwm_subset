...
cat(paste("Rscript nwm_output/bin/cull_catchments.R nwm_output/NWM_v1.2_nc_tools_v1/spatialMetadataFiles/nwm-v1.2-channel_spatial_index.nc culled_latlon.nc", 
					paste0("nwm_output/nexus_locations/",comids,"\n",
...
for(date in dates) {
  ...
  for(run in runs) {
    ...
    cat(paste("  bash nwm_output/bin/fmrc_converter.sh", in_spec, out_file, temp_file_dir, "nwm_output/bin/", paste0("nwm_output/nexus_locations/",comids,"\n")))
  }  
  cat(paste0("fi\n"))
}
sink()
