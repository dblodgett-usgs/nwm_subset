...
if ls $in_dir/nwm.${r}.*  1> /dev/null 2>&1; then
  ...
  for f in ${tt}/*; do
    ...
    Rscript --vanilla cull_catchments.R $f $f comids.rds
  done
  ff=${temp_dir_use}/${r}.nc
  ncrcat -h ${tt}/* $ff > /dev/null 2>&1 # This is not copying the scale_factor attribute!!!
  # rm -r ${tt}
...
rm $file_list
ncks -h -A -v latitude,longitude culled_latlon.nc ${temp_dir}/$out_file
