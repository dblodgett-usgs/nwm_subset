...
Rscript ${project_root}nwm_cull/cull_catchments.R ${project_root}NWM_v1.2_nc_tools_v1/spatialMetadataFiles/nwm-v1.2-channel_spatial_index.nc ${latlon_path}culled_latlon.nc ${comid_list}
...
for f in ${base_path}*.nc4; do
  ...
  out=${out_path}${ff:0:${#ff}-1}
	...
	if [ -f $out.tmp ] || [ -f $out ]; then
	  ...
	  Rscript --vanilla ${cull_nwm_path}cull_catchments.R $tmp $tmp ${comid_list}
